...
-- 工时管理菜单
INSERT INTO tb_menu (menu_code, menu_name, parent_id, menu_type, menu_url, menu_icon, sort_order, status) VALUES
('workhour:config', '工时配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour') AS tmp), 2, '/workhour/config', 'bi-sliders', 1, 1)
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);

-- 工时配置按钮权限
INSERT INTO tb_menu (menu_code, menu_name, parent_id, menu_type, menu_url, menu_icon, sort_order, status) VALUES
('workhour:config:add', '新增配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour:config') AS tmp), 3, NULL, 'bi-plus', 1, 1),
('workhour:config:edit', '编辑配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour:config') AS tmp), 3, NULL, 'bi-pencil', 2, 1),
('workhour:config:delete', '删除配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour:config') AS tmp), 3, NULL, 'bi-trash', 3, 1),
('workhour:config:approve', '审批配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour:config') AS tmp), 3, NULL, 'bi-check-lg', 4, 1),
('workhour:config:invalidate', '作废配置', (SELECT id FROM (SELECT id FROM tb_menu WHERE menu_code='workhour:config') AS tmp), 3, NULL, 'bi-slash-circle', 5, 1)
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);

-- 初始化角色菜单权限（root 角色拥有所有权限）
INSERT INTO tb_role_menu (role_id, menu_id)
SELECT r.id, m.id FROM tb_role r, tb_menu m WHERE r.role_code = 'root'
